      ...
      {filteredWords.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center space-y-3">
          ...
          <h3 className="font-semibold text-slate-700 text-base">
            {searchTerm ? '未找到符合条件的生词' : '生词本暂无记录'}
          </h3>
          <p className="text-slate-400 text-xs max-w-sm mx-auto">
            {searchTerm ? '尝试更换搜索关键字' : '在单词测试中答错的词汇会自动记录在这里，也可以通过【导入词汇】快捷批量添加。'}
          </p>
        </div>
      ) : (
            ...
            {paginatedWords.map((item) => (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  {/* Error count badge */}
                  <div className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-bold shrink-0 ${
                    (item.errorCount || 0) > 0 
