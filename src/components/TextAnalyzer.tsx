...
💡 提示：系统会自动过滤文章标题与非词汇干扰，准确提取有效英文单词；分析时将由 AI 智能补全词性、音标、精美例句与测验题。`}
                    rows={8}
                    className="w-full p-3.5 rounded-xl border border-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 outline-none resize-none text-slate-700 text-sm leading-relaxed transition-all placeholder:text-slate-400"
                  />
                </div>
                ...
                <div
                  ...
                  className={`border-2 border-dashed rounded-xl p-4 text-center cursor-pointer transition-colors flex items-center justify-center gap-3 ${
                    isDragOver ? 'border-indigo-500 bg-indigo-50/50' : 'border-slate-200 bg-slate-50/50 hover:bg-slate-100/80'
                  }`}
                >
                  <input
                    ref={fileInputRef}
...
💡 提示：系统会自动忽略“Welcome Unit”、“按通用教材整理”等非词汇行，准确提取有效英文单词；系统将由 AI 智能自动补全词性、音标、精美例句与测验题。`}
                    rows={9}
                    className="w-full p-3.5 rounded-xl border border-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 outline-none resize-none text-slate-700 text-xs sm:text-sm leading-relaxed transition-all placeholder:text-slate-400 font-mono"
                  />
                </div>
              </>
            )}
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs space-y-5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-medium text-sm rounded-xl shadow-md shadow-emerald-100 transition-all cursor-pointer"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[520px] overflow-y-auto pr-1">
            {extractedWords.map((item, idx) => (
              ...
              >
                ...
                {/* Chinese definition */}
                <div className="text-base font-bold text-indigo-900 bg-indigo-50/70 py-2 px-3 rounded-xl border border-indigo-100/80 leading-snug">
                  {item.chinese}
                </div>
